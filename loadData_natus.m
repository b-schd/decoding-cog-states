...
% parameters
ddir = '/Users/drew/Data/CCDT/eeg/'; % data directory
subj = 'HUP152'; % subject
sess = '10Nov17'; % session date
...
% load params.txt
ref_pth = fullfile(ddir, subj, ['/eeg.' reref]);
fid = fopen(fullfile(ref_pth, 'params.txt'));
C = textscan(fid,'%s %s');
fclose(fid);
...
% load jacksheet.txt
doc_pth = fullfile(ddir, subj, 'docs');
fid = fopen(fullfile(doc_pth, 'jacksheet.txt'));
J = textscan(fid,'%d %s');
fclose(fid);
...
% parse file names
if ~isempty(stime)
    fnm = dir(fullfile(ref_path, [subj '_' sess '_' stime '*']));
else
    fnm = dir(fullfile(ref_pth, [subj '_' sess '*']));
end
Nfile = length(fnm);
chnum = []; stimes = [];
for ii = 1:Nfile
...
for ii = 1:Nfile
    ...
    fid = fopen(fullfile(fnm(ii).folder, cfnm),'r');
